document.addEventListener('DOMContentLoaded', function () {
    ...
    const servicioSelect = document.getElementById('sala-select');
    ...
    const ingresosFormateados = atencionesActivas.map(atencion => {
        return {
            id: atencion.id,
            ...
            idPaciente: atencion.paciente_id,
        };
    });

    console.log(ingresosFormateados);
    ...
    function formatearTexto(dni, nombre, expediente, fechaAtencion) {
        // Asegurar que dni tenga 13 caracteres, rellenando con espacios si es necesario
        dni = dni ? dni.slice(0, 13).padEnd(13, ' ') : ' '.repeat(13);
        
        // Normalizar nombre a 20 caracteres: cortar si excede, rellenar si es menor
        ...
        return `${expediente} |${dni} |${nombre} |${fechaAtencion}`;
    }
    ...
    function mostrarLista(lista, elemento) {
        ...
        lista.forEach((registro, index) => {
            ...
            li.textContent = formatearTexto(registro.dni, registro.nombre, registro.exp,registro.fechaAtencion);
    ...
    function filtrarMostrar() {
        ...
        pendientes.sort((a, b) => a.nombre.localeCompare(b.nombre, 'es', { sensitivity: 'base' }));
        porRecibir.sort((a, b) => a.nombre.localeCompare(b.nombre, 'es', { sensitivity: 'base' }));
    
        
        // Si la sala seleccionada es 'todos', no filtramos. Si no, filtramos por la sala
        if (servicio === 'todos') {
            filtrados = pendientes;  // No filtramos, mostramos todos
            filtradosPorRecibir = porRecibir;
        } else {
            filtrados = pendientes.filter(expediente => expediente.idServicio == servicio);  // Filtramos por sala
            filtradosPorRecibir = porRecibir.filter(expediente => expediente.idServicio == servicio);  // Filtramos por sala
    ...
    busquedaPendientes.addEventListener('input', () => {
        ...
        timeout = setTimeout(() => {
            ...
            let filtered = pendientes.filter(exp => {
                ...
                const coincideServicio = servicio === 'todos' || exp.idServicio == servicio;
    ...
    busquedaPendientes.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            ...
            const indice = pendientes.findIndex(expediente => {
                ...
                const coincideServicio = servicio === 'todos' || expediente.idServicio == servicio;
                return coincideDni && coincideServicio;
            });
            ...
            if (indice !== -1) {
                ...
                alert(`No se encontró ningún registro con DNI: ${valor}`);
                busquedaPendientes.value = '';
                filtrarMostrar(); // actualizar vistas
            }
        }
    ...
    busquedaPorRecibir.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            ...
            // Buscar en la lista porRecibir por DNI
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        const confirmado = await confirmarAccion(
            '¿Estás seguro?',
            `Estás a punto confirmar la recepcion de ${porRecibir.length} atenciones`
        );
        if (confirmado) {
    ...
    async function enviarRecepcion() {
        ...
        try {
            const response = await fetch(urls['registrarRecepcionAtenciones'], {
                method: "POST",
                body: JSON.stringify({
                    observaciones: observaciones.value, // pa probar
                    atenciones: porRecibir // lista de diccionarios 
                }),
                headers: {
    ...
    servicioSelect.addEventListener('change', filtrarMostrar);
