document.addEventListener('DOMContentLoaded', function () {
    ...
    const ingresosFormateados = ingresosActivos.map(ingreso => {
        return {
            ...
            idSala: ingreso.sala__id,
            dni: ingreso.paciente__dni || '',
            exp: ingreso.expediente_numero,
            fechaIngreso: ingreso.fecha_ingreso,  // Puede ser null
    ...
    function formatearTexto(dni, nombre, expediente, fechaIngreso) {
        ...
        return `${expediente} |${dni} |${nombre} |${fechaIngreso}`;
    }
    ...
    function mostrarLista(lista, elemento) {
        ...
        lista.forEach((registro, index) => {
            ...
            li.textContent = formatearTexto(registro.dni, registro.nombre, registro.exp,registro.fechaIngreso);
    ...
    function filtrarMostrar() {
        const sala = salaSelect.value;  // Obtenemos la sala seleccionada
        ...
        if (sala === 'todos') {
            ...
            filtrados = pendientes.filter(expediente => expediente.idSala == sala);  // Filtramos por sala
            filtradosPorRecibir = porRecibir.filter(expediente => expediente.idSala == sala);  // Filtramos por sala
    ...
    busquedaPendientes.addEventListener('input', () => {
        ...
        timeout = setTimeout(() => {
            ...
            let filtered = pendientes.filter(exp => {
                ...
                const coincideSala = sala === 'todos' || exp.idSala == sala;
                return coincideTexto && coincideSala;
            });
    ...
    busquedaPendientes.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            ...
            const indice = pendientes.findIndex(expediente => {
                ...
                const coincideSala = sala === 'todos' || expediente.idSala == sala;
                return coincideDni && coincideSala;
            });
    ...
    async function enviarRecepcion() {
        ...
        try {
            const response = await fetch(urls['registrarRecepcionIngresos'], {
                method: "POST",
                body: JSON.stringify({
            ...
            toastr.error("Hubo un error inesperado al registrar la recepcion.");
        }
    }

    salaSelect.addEventListener('change', filtrarMostrar);
